...
1. **Azure Function hosting model:** same App Service Plan as the main API. This is the cheapest option — no additional Consumption Plan costs. Telemetry ingestion volume is low in the initial deployment (tens of equipment, not thousands). If burst ingestion becomes a problem, the Function can be moved to a Consumption Plan later with a configuration change. The current `TelemetryIngestionFunction` is lightweight (HTTP trigger → Dapper insert → dead-letter) and does not need auto-scaling.
2. **Alert evaluation async mechanism:** direct service call within the Function. The `AlertEvaluatorService` is already implemented as a synchronous method that evaluates thresholds and creates `Alert` entities. Adding a Channel or in-memory queue between ingestion and evaluation adds complexity without benefit — the evaluation is fast (in-memory threshold comparison) and the Function is already async. If evaluation latency becomes a concern, a queue can be introduced later.
3. **Migration of existing `TelemetryEvent` data:** accept the gap for v1. The application is pre-production — the existing telemetry dataset is seed data only (~70 records from `DataSeeder`, 7 days × 10 equipment). A backfill migration is unnecessary. The EF migration that adds `OrganizationId` to `TelemetryEvent` can set a default value or populate from `Equipment.OrganizationId` via a SQL join. For production, the dataset will start fresh with the correct schema.
